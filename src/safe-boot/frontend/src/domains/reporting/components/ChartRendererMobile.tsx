/**
 * ChartRenderer - Composant graphique pour rapports (version React Native)
 * Rendu natif (View/Text) sans dépendance externe : recharts exige react-dom
 * (web) et react-native-svg-charts est obsolète pour RN 0.76.
 * @traceable flexible-report-engine-arch.md §ChartRenderer Mobile
 */
...
// Types de graphiques mobile
export type ChartType = 'bar' | 'line' | 'pie';

// Props du composant mobile
...
// Données de démonstration quand report.data n'est pas structuré par catégorie.
const FALLBACK_DATA: ChartDatum[] = [
  { name: 'Cat 1', income: 600, expense: 400 },
  { name: 'Cat 2', income: 400, expense: 300 },
  { name: 'Cat 3', income: 300, expense: 200 },
];
...
export function ChartRendererMobile({ data, type, config = {}, style }: ChartRendererProps) {
  const { title = 'Graphique', colorScheme = DEFAULT_COLOR_SCHEME } = config;
  const chartData = extractChartData(data);
  const maxValue = Math.max(1, ...chartData.map((d) => Math.max(d.income, d.expense)));

  // Graphique à barres (horizontales, natif)
  const renderBar = () => (
  ...
  // Graphique en ligne : liste des séries avec variation (natif)
  const renderLine = () => (
    <View style={styles.chartArea}>
      {chartData.map((d, index) => (
  ...
  // Graphique en camembert : segments proportionnels (natif)
  const renderPie = () => {
    const total = chartData.reduce((sum, d) => sum + d.income, 0) || 1;
    return (
      <View style={styles.chartArea}>
        {chartData.map((d, index) => (
          <View key={d.name} style={styles.pieRow}>
            ...
            <Text style={styles.pieValue}>{Math.round((d.income / total) * 100)}%</Text>
          </View>
        ))}
      </View>
    );
...
const styles = StyleSheet.create({
  container: { padding: 16, backgroundColor: 'white', borderRadius: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 3, elevation: 2 },
  title: { fontSize: 16, fontWeight: '600', color: '#374151', marginBottom: 12 },
  chartContainer: { flex: 1, height: 250 },
  ...
  lineLabel: { width: 80, fontSize: 12, color: '#374151' },
...
export default ChartRendererMobile;
